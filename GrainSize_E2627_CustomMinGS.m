...
% Area based grain size measurement according to ASTM E2627 but with no
% specific grain size threshold
...
[~, ~, ~, ~, ~, ~, inside_grains, edge_grains, ~] = grainsize_areas_planimetric(ebsd, polygon, varargin{:});
...
if ismember('PlotResults', varargin)
    ...
    plot(edge_grains, 'FaceColor', 'w', 'FaceAlpha', 1.0); hold on
    plot(edge_grains.boundary, 'linewidth', 1.5, 'lineColor', 'black'); hold on
    plot(excluded_grains, 'FaceColor', 'black', 'FaceAlpha', 1); hold on
    plot(inside_grains.boundary,'linewidth', 2, 'lineColor', 'white'); hold off
end
